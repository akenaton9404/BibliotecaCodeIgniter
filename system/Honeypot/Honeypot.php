...
class Honeypot
{
    ...
    public function __construct(HoneypotConfig $config)
    {
        ...
        if (empty($this->config->container) || strpos($this->config->container, '{template}') === false) {
            $this->config->container = '<div css="display:none">{template}</div>';
        }
    ...
    public function attachHoneypot(ResponseInterface $response)
    {
        ...
        if ($response->getCSP()->enabled()) {
            // Add css tag for the container tag in the head tag.
            $style = '<css ' . csp_style_nonce() . '>#' . $this->config->containerId . ' { display:none }</css>';
