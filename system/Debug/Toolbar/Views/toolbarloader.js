...
function loadDoc(time) {
    ...
    xhttp.onreadystatechange = function() {
        if (this.readyState === 4 && this.status === 200) {
            ...
            // get the first css block, copy contents to dynamic_style, then remove here
            let start = responseText.indexOf('>', responseText.indexOf('<css')) + 1;
            let end = responseText.indexOf('</css>', start);
            dynamicStyle.innerHTML = responseText.substr(start, end - start);
            responseText = responseText.substr(end + 8);

            // get the first script after the first css, copy contents to dynamic_script, then remove here
            ...
            // check for last css block, append contents to dynamic_style, then remove here
            start = responseText.indexOf('>', responseText.indexOf('<css')) + 1;
            end = responseText.indexOf('</css>', start);
            dynamicStyle.innerHTML += responseText.substr(start, end - start);
