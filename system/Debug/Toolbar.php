...
class Toolbar
{
            ...
            if (is_array($items)) {
                foreach ($items as $key => $value) {
                    if (is_string($value)) {
                        ...
                        $kint = substr($kint, strpos($kint, '</css>') + 8);
    ...
    protected function renderTimelineRecursive(array $rows, float $startTime, int $segmentCount, int $segmentDuration, array &$styles, int &$styleCount, int $level = 0, bool $isChild = false): string
    {
        ...
        foreach ($rows as $row) {
            ...
            $output .= '<td class="' . ($isChild ? 'debug-bar-width30' : '') . '" css="--level: ' . $level . ';">' . ($hasChildren || $isQuery ? '<nav></nav>' : '') . $row['name'] . '</td>';
            ...
            // Add children if any
            if ($hasChildren || $isQuery) {
                $output .= '<tr class="child-row" id="timeline-' . ($styleCount - 1) . '_children" css="' . ($open ? '' : 'display: none;') . '">';
                ...
                if ($isQuery) {
                    ...
                    $output .= '<tr>';
                    $output .= '<td class="query-container" css="--level: ' . ($level + 1) . ';">' . $row['query'] . '</td>';
                    $output .= '</tr>';
                } else {
    ...
    public function prepare(?RequestInterface $request = null, ?ResponseInterface $response = null)
    {
        ...
        if (CI_DEBUG && ! is_cli()) {
            ...
            Kint::$mode_default = $oldKintMode;
            $kintScript         = substr($kintScript, 0, strpos($kintScript, '</css>') + 8);
            ...
            $script = PHP_EOL
                ...
                . '<css ' . csp_style_nonce() . ' id="debugbar_dynamic_style"></css>'
                . $kintScript
                . PHP_EOL;
